...
public class GetUserListRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public GetUserListRestAction(Configuration config, Settings settings, RestController controller){
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext()+"/user/list", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException{
		...
		return action.listUser(userDao, request);
	}
