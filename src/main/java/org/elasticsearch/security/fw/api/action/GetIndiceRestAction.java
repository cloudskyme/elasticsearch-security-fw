...
public class GetIndiceRestAction extends BaseAction {

	Configuration config;

	@Inject
	public GetIndiceRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/indices", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.viewUserIndices(userIndicesDao, request);
	}
}
