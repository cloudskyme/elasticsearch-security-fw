...
public class GetIndiceStatsRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public GetIndiceStatsRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/indices/stats", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.statsUserIndices(userIndicesDao, request);
	}
