...
public class GetAclListRestAction extends BaseAction {

	Configuration config;

	@Inject
	public GetAclListRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/acl/list", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.listUserAcl(userAclDao, request);
	}
}
