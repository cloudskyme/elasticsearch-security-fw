...
public class PutUserRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public PutUserRestAction(Configuration config, Settings settings, RestController controller){
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.PUT, config.getApiContext()+"/user", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.updateUser(userDao, request);
	}
