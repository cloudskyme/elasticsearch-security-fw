...
public class GetIndiceListRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public GetIndiceListRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/indices/list", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.listUserIndices(userIndicesDao, request);
	}
