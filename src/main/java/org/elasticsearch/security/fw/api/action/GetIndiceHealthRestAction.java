...
public class GetIndiceHealthRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public GetIndiceHealthRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/indices/health", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.healthUserIndices(userIndicesDao, request);
	}
