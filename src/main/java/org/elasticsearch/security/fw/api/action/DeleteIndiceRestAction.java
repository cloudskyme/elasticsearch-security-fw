...
public class DeleteIndiceRestAction extends BaseAction {

	Configuration config;

	@Inject
	public DeleteIndiceRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.DELETE, config.getApiContext() + "/indices", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.removeUserIndices(userAclDao, userIndicesDao, request);
	}
