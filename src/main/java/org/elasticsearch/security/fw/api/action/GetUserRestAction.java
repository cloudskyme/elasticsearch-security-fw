...
public class GetUserRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public GetUserRestAction(Configuration config, Settings settings, RestController controller){
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext()+"/user", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException{
		...
		return action.viewUser(userDao, request);
	}
	
	protected Set<String> responseParams() {
		return new HashSet<String>(Arrays.asList("username"));
    }
}
