...
public class GetAclRestAction extends BaseAction {

	Configuration config;

	@Inject
	public GetAclRestAction(Configuration config, Settings settings, RestController controller) {
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.GET, config.getApiContext() + "/acl", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.viewUserAcl(userAclDao, request);
	}
