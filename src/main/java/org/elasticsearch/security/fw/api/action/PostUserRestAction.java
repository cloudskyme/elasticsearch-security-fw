...
public class PostUserRestAction extends BaseAction {
	Configuration config;
	
	@Inject
	public PostUserRestAction(Configuration config, Settings settings, RestController controller){
		super(settings);
		this.config = config;
		controller.registerHandler(RestRequest.Method.POST, config.getApiContext()+"/user", this);
	}
	...
	protected BytesRestResponse processRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return action.addUser(userDao, request);
	}
